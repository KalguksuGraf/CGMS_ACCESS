...
function CgmDtlsChart(_param){
	...
	var svg = d3.select(param.div).append("svg")
		...
		.on("click", function(){
			if($("#pop_chart").length > 0){
				$("#pop_chart .popup_value li:eq(1)").html(Math.round(param.minMax[1]) + ' <em>mg/dL</em>');
				
				$("#pop_chart .popup_chart_area").empty();
				
				var cgmPopChart = new CgmPopChart({
					  "div"			: "#pop_chart .popup_chart_area"
					, "width"		: $("#pop_chart .popup_chart_area").width()
					, "height"		: $("#pop_chart .popup_chart_area").height()
					, "minMax"		: param.minMax
					, "columns"		: [ "y" ]
					, "baseColumns"	: "x"
					, "colors"		: param.colors
