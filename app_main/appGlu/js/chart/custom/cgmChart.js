...
function CgmChart(_param){
    ...
    .attr("stroke-opacity", 1)
    .attr("d", function(d){return lineGenerator([[0, -height-param.margin.top], [0, 0]]);} )
    ;
...
//		.data(rawData)
	...
	this.insertTypeData = function(rawData){
		...
		var typeLine = typeGrp.selectAll(".typeLine")
		    ...
		    .attr("d", function(d){
		    	...
		    	return lineGenerator([[x(xVal), -param.margin.top+10], [x(xVal), y(d3.min(param.minMax))]]);
		    })
	    ;
	 ...
	 * 활동량 데이터 삽입
	 **********************/
	this.insertActData = function(rawData){
		var gradient = svg.append("linearGradient")
			.attr("id", "bar_gradientAct")
			.attr("x1", "0%")
			.attr("y1", "100%")
			.attr("x2", "0%")
			.attr("y2", "0%");
		...
		var barGroup = svg.selectAll(".bar")
			...
			.attr("class","bar")
			.style("fill", function(d, i) {
				return "url(#bar_gradientAct)";
			})
			.attr("x", function(d){
		    	var xVal = Number(d.hour) * 60;
		    	return x(xVal - 15);
			})
			.attr("rx", 3)
			.attr("ry", 3)
			.attr("y",  function(d,i) {
				return y((d.y) + d3.min(param.minMax));
			})
			.attr("width", 6)
