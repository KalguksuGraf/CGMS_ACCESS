...
function AvgCalChart(_param){
	...
	var textClass = param.classname
	//도넛챠트
	var donut = param.donut||{};
	donut.div = donut.div||"body";
	donut.size = donut.size||15;
	...
	this.insertData = function(rawData){
		...
		targetCont = isNullToString(rawData[1].targetcont,0);
	...
	this.initChart = function(){
	    ...
	    donutChart
		.append('text')
		.attr('class','text '+textClass)
		.attr('x',0)
		.attr('y',5)
		.text(targetCont+" 명")
		.attr("text-anchor", "middle")
		.style("fill",function(){
			if($('#'+textClass).hasClass('tab_on')){
				if(textClass == 'LOW_EMER'){
					return '#56028F';
				}else if(textClass == 'HIGH'){
					return '#FE9504';
				}else if(textClass == 'DATA_DISC'){
					return '#DC0000';
				}
			}
			return 'black';
