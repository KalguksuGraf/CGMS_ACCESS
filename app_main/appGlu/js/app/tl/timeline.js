			...
			for(var i = 0; i < timelineList.length; i++){
				...
				if(type == "BS"){					
					li = $('<li class="' + singleCls + '">'
							...
							+ '</li>'
							+ '<li>'
							+ '<a href="" onclick="moveEditPage(\'' + editUrl + '\');" class="mod ui-btn-icon-right">수정</a>'
							+ '</li>'
							+ '</ul>'
							+ '</button>'
							+ '</div>'
...
// 수정 화면 이동
function moveEditPage(path){
	localStorage.setItem("returnPage", "timeLine");
	localStorage.setItem("timelineDe", curDate);
	location.href = path;
}
